...
데이터 나열이 아니라 **읽는 사람이 결정을 내릴 수 있는 분석**을 쓴다. 아래 다섯 규율이
제목, 숫자, 차트, 권고, 한계를 한 줄로 묶는다.

1. **액션 타이틀** — 섹션과 Panel 제목은 명사구가 아니라 주장문으로 쓴다. 제목만 읽어도
   결론이 이어지도록. TOC가 제목을 그대로 보여주므로 제목 나열이 곧 요약이 된다.
   전: `MRR 추세` → 후: `확장이 신규를 대체했다`
2. **비교 없는 숫자 금지** — 모든 KPI와 본문 수치는 기준을 동반한다(전기 대비, 목표
   대비, 동종 대비). `KpiRow`의 `delta`/`compare` 필드가 이 규율의 구현이다.
   전: `MRR $1.42M` → 후: `MRR $1.42M` + `compare: 전주 $1.377M 대비`
3. **so-what 한 줄** — 차트와 표는 Panel `description` 또는 직후 문단 첫 문장에서
   '이 그림이 말하는 것'을 선언한다. 데이터 소개(`일별 추이를 나타냈다`)는 금지 표현.
   전: `주간 마감 MRR을 나타냈다` → 후: `현재 속도면 $1.5M까지 약 3주다`
4. **권고의 3요소** — 권고 Claims는 실행 주체, 시점, 성공 판정 기준을 담는다.
   전: `모니터링을 강화한다` → 후: `플랫폼팀이 8월 중 counterKey 단위 대시보드를
   만들고, 주간 거부율 5% 이하를 기준으로 삼는다`
5. **한계 선제 처리** — 해석의 약점(표본, 기간, 교란 요인)을 결론 근처에서 문서가 먼저
   말한다. 신뢰는 완벽 주장이 아니라 반론 처리에서 나온다.
   전: (침묵) → 후: `한 주 스냅샷과 상위 계정 표본에 기대며 시즌성은 분리하지 않았다`

**서사 규율은 섹션을 늘리는 이유가 아니다 — 빈약한 해석을 고칠 때 쓰고, 채우기용
문장과 패널은 스파인 규칙대로 버린다.**
